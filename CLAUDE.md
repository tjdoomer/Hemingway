...
After completing work on a worktree branch, a security scan MUST be run:
```bash
cd /Users/thomjames-worrall/Security-Bot
security-agent scan /path/to/worktree --depth standard --output ./reports
```

- **CRITICAL/HIGH findings** — MUST be fixed before merge. No exceptions.
- **MEDIUM findings** — Should be fixed. If not, document in PLAN.md with timeline.
- **LOW/INFO** — Note in CHANGELOG.md.

See `.hemingway/prompts/SECURITY-REVIEW.md` for the full security review process.
...
| File | Purpose |
|------|---------|
| `PLAN.md` | Master task list — check off work as you complete it |
| `CHANGELOG.md` | All changes with commit hashes and agent names |
| `CLAUDE.md` | This file — mandatory rules for all agents |
| `.hemingway/prompts/` | Specialist prompts for each task with code examples |
| `.hemingway/security-reports/` | Security scan results |
| `SECURITY_FIXES.md` | Documentation of security vulnerabilities fixed |
